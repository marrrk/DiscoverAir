...
//#include <~/LoRaDongle/litex/litex/litex/soc/software/libbase/i2c.h>
...
void discoverAir(void) {

    static const uint8_t HDC1080_ADDR = 0x40; // HDC1080 address
    uint8_t receive_data[4];  //  register that is used to store data from the sensor. Temperature and Humidity data are stored in the same register
    unsigned char send_data =0x00; // command send to HDC1080 for it to start  measuring
    uint16_t temp_x,humi_x;    
   

    while (1) {
        /* To measure temnperature and humidity you need to send the 7 bit slave address and  an extra bit which will be zero, the next step is to send a 0x00 
         *command for the smsor to start reading. 
         */    
   
        i2c_start();
        bool addr =i2c_transmit_byte(HDC1080_ADDR<<1);
        
        if ( addr == true ){
		bool pointer = i2c_transmit_byte(send_data);
         	i2c_stop();
                if ( pointer == true){
                    bool read =  i2c_read(HDC1080_ADDR,send_data,receive_data,4,false); // take the reading 
                    msleep(1000);
                   ...
                   if (read == true){
                       ...
                       int temperature1= ((temp_x/65536.0)*165.0)-40.0;
                       int humidity=((humi_x/65536.0)*100.0);
                       printf("Temperature in degrees celcius is: %d", temperature1);
                       printf("\n");
                       printf("Humidity is: %d",humidity);
                       printf("\n");
                   }
                  else{
                      printf("The read was unsuccessful");

                   }
                  }
                else{
                     printf("The command to start measuring was unsucceful");
                    }
         }
        else
        {
